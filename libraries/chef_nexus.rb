...
class Chef
  module Nexus
    ...
    SSL_FILES_DATABAG     = 'nexus_ssl_files'.freeze
    ...
    class << self
      ...
      def nexus(node)
        ...
        if Chef::Config['solo']
          ...
          credentials = if node['nexus']['cli']['default_admin_credentials_updated']
                          credentials_entry['updated_admin']
                        else
                          credentials_entry['default_admin']
                        end
      ...
      def encrypted_data_bag_for(node, data_bag)
        ...
        return @encrypted_data_bags[data_bag] if @encrypted_data_bags[data_bag]
        raise Nexus::EncryptedDataBagNotFound, data_bag
      end
